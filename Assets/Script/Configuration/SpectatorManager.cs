...
public class SpectatorManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        modeDropdown.onValueChanged.AddListener((int value) => {
            if (SpectatorCam == null || cameraSmooth == null ||
                SpectatorFPTarget == null || SpectatorTPTarget == null)
                return;
            ...
            switch (value)
            {
                ...
                case 1:
                    ...
                    SpectatorCam.cullingMask |= 1 << LayerMask.NameToLayer("TPBlock");
                    SpectatorCam.cullingMask &= ~(1 << LayerMask.NameToLayer("FPBlock"));
                    break;
        ...
        fpsDropdown.onValueChanged.AddListener((int value) => {
            ...
            int spectatorFps = int.Parse(fpsString.Remove(0, 3));

            if (cameraSmooth != null)
                cameraSmooth.SetSpectatorFPS(spectatorFps);
        });
    ...
    void ApplyTPCamTransform()
    {
        ...
        SpectatorTPTarget.position = new Vector3(
            ConfigManager.config.TPCamPosition[0],
            ConfigManager.config.TPCamPosition[1],
            ConfigManager.config.TPCamPosition[2]
        );

        SpectatorTPTarget.rotation = Quaternion.Euler(
            ConfigManager.config.TPCamRotation[0],
